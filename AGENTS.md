...
- **Practice keyboard flow** — `Ctrl/Cmd+Enter` submits an answer (or advances from Drill feedback); `Alt+M` starts/stops voice recording. Bare Enter and Space remain available for writing, and question replay remains button-only to avoid browser shortcut conflicts.
- **Practice voice controls** — Voice capture produces an editable live draft. The UI intentionally does not expose Download Audio or Improve Transcript; use **Get Answer Tips** to improve the substance of a draft before submitting. A small violet **Replay** control (`.interview-replay-btn`, inside `mq-interview-css`) toggles playback of the just-recorded answer straight from the browser-memory Blob — no server round-trip, nothing written to disk or to `AppState`. Voice audio is retained for the current answer turn only and released by `cancel_recording()`/`dispose()` on submit, discard, or a new recording; transcription success deliberately no longer nulls `lastBlob`, since that used to destroy the audio the instant the transcript arrived.
