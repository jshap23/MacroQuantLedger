...
def render_briefing_strip(state: AppState, save_indicator, compact: bool = False):
    def save():
        save_state(state)
        save_indicator()
    ...
    if compact:
        with ui.element("div").classes("today-top-of-mind"):
            with ui.column().style("gap:0; min-width:105px;"):
                ui.label("TOP OF MIND").classes("today-top-of-mind-label")
                age_label = ui.label(age_text).style(
                    f"color:{age_color}; font-size:0.62rem; font-weight:700; white-space:nowrap;"
                )
            ta = ui.textarea(
                value=b.top_of_mind,
                placeholder="Lead thought or key watch item…",
            ).classes("dark-input today-top-of-mind-input")
    else:
        with ui.row().style(
